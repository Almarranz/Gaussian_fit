...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 13 13:50:56 2021
...
for sloop in range(len(step)):
    ...
    plt.plot(xplot, gaussian(xplot, mean[0], mean[1], mean[2]) , color="darkorange", linewidth=3, alpha=1)
...
#for file in range(1,4):
    if sloop==0:
        with open (pruebas+'%s_vy_gauss_var.txt'%(zone), 'w') as f:
            f.write(' %.4f %.4f %.4f %.0f %s'%(mean[0], mean[1], mean[2],results['logz'][-1],nbins)+'\n')
    else:
